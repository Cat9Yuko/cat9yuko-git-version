        ...
        success: function(json) {
            if (json.returncode === 999) {
                layer.alert("请登录后再进行答题！", {
                    ...
                    location.href = cxtPath + "login_app.jsp";
                });
            } else {
                isAnswered();
            }
        ...
        success: function (json) {
            if (json.returncode === 10001 || json.returncode === 10002) {
                ...
                layer.alert(json.message, {
                    ...
                    window.location.href = cxtPath + "score_app.jsp?score=" + json.result.score +"&answerid=" + answerId;
                });
...
function loadQuestion() {
	 $("body").showLoading();
    var data = {
        "answerId": $("body").data("answerid"),
        "paperId": $("body").data("paperid")
    };
    $.ajax({
        type: "POST",
        url: getRootPath() + "/handle/dt/loadRandomQuestion",
        data: data,
        dataType: "json",
        success: function (json) {
            if (json.returncode === 0) {
               // $(".jbqk p").text(json.result.answer.name);
                $("#DataTmpl").tmpl(json.result).appendTo("#container");
                //isAnswered();
            } else {
            }
			  $("body").hideLoading();
    ...
    $.ajax({
        ...
        success: function(json) {
            if (json.returncode === 0) {
                ...
                var answerid = $("body").data("answerid");
                window.open(cxtPath + "score_app.jsp?score=" + result.score+"&answerid=" + answerid, "_self");
            } else {
