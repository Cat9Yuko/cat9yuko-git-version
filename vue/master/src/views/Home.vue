<template>
  <base-page class="home-wrap">
    <van-nav-bar right-arrow title="部网站信息发布审核单" right-text="新建" @click-left="onClickLeft" @click-right="onClickRight">
<!--      <van-icon name="icon iconfont icon-ai207" slot="left" />-->
<template #left>
    <van-icon name="search" size="0.42667rem" />
  </template>
    </van-nav-bar>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      onClickRight() {
        this.$router.push({ name: 'NewTask', query: {} })
      },
      // 新增主页搜索按钮
      onClickLeft() {
        this.$router.push({ name: 'MySearch', query: {} })
      }
    }
  }
</script>
