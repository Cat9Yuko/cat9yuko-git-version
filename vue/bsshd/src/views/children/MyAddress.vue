<template>
    <div class="my-address">
        <van-nav-bar right-arrow title="足迹" @click-left="onClickLeft" @click-right="onClickRight">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      renderTaskList(args = {}) {
        this.$http.post(this.$API.SLB_POST_FOOTMARK_LIST, this.pageInfo).then((res) => {
      ...
      delItem(id) {
        Dialog.confirm({
          ...
          this.$http.post(this.$API.SLB_POST_FOOTMARK_DEL, { id: id }).then(res => {
            Toast.success('删除成功')
            this.renderTaskList()
          }).catch((e) => {
            // console.log(e)
